...
import fds.ui.common.EpochListTableRowViewModel;
import fds.ui.common.OutputListTableRowViewModel;
import fds.ui.common.OutputListViewCellController;
...
public class PreciseOrbitPredictionController extends BorderPane implements Initializable {
  ...
  @FXML
  private Button preNewWindow;
  @FXML
  private Button preSave;
  @FXML
  ...
  public void initialize(URL location, ResourceBundle resources) {
    handler = new TextViewerEventHandler(this, preMainTab);
...
//    });
    ...
    preNewWindow.addEventHandler(MouseEvent.MOUSE_CLICKED, handler);
    preSave.addEventHandler(MouseEvent.MOUSE_CLICKED, handler);
    prePrint.addEventHandler(MouseEvent.MOUSE_CLICKED, handler);
  }
...
//          System.out.println(model.getId());
      ...
      stationDataGridPane.add(model.getChecked(), 1, i);
      stationDataGridPane.add(new Label(model.getId()), 2, i);
      stationDataGridPane.add(new Label(model.getName()), 3, i);
    }
  ...
  private void setOutputList(PreciseOrbitPredictionViewModel viewModel) {
    ...
    for (int i = 0; i < viewModel.outputListTableRowViewModelList.size(); i++) {
      ...
      outputListGridPane.add(new Label(model.itemProperty().get()), 1, i);
      outputListGridPane.add(model.generationProperty(), 2, i);
      outputListGridPane.add(new Label(model.fileNameProperty().get()), 3, i);
      outputListGridPane.add(controller.getViewButton(), 4, i);
